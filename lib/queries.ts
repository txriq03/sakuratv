...
export const getTrending: any = async () => {
  ...
  try {
    const {
      data: { results },
    } = await axios.get(url); // Store the response object here
...
export const getEpisodeStreamingLink = async (episodeId: any) => {
  const url =
    ...
    "/api/v2/hianime/episode/sources?animeEpisodeId=" +
    episodeId +
    "&server=hd-2";
  //   const url =
  //     "https://aniwatch-api-cyan.vercel.app/api/v2/hianime/episode/sources?animeEpisodeId=steinsgate-3?ep=230&server=hd-2";
  try {
    const {
      data: { data: episodeData },
    } = await axios.get(url);
...
export const getAnimeInfo: any = async (animeId: string) => {
  const url =
    process.env.NEXT_PUBLIC_API_URL + "/api/v2/hianime/anime/" + animeId;
  try {
